...
export class FileManager {
   ...
   */
  private sanitizeFolderName(task: string): string {
   ...
   */
  private parseCodeFiles(codeMarkdown: string): Array<{ filePath: string; content: string }> {
  ...
  private inferExtensionFromContent(content: string): string {
    ...
    if (content.includes("@media") || content.includes("{")) {
      ...
      if (content.match(/[.#][\w-]+\s*{/)) return ".css";
    }
    if (content.includes("import ") && content.includes("from ")) return ".js";
    if (content.includes("require(")) return ".js";
    if (content.includes("def ") || content.includes("import ")) {
      if (content.includes("print(") && !content.includes("console.")) return ".py";
    }
   ...
   */
  async writeFiles(task: string, codeMarkdown: string): Promise<{ folderPath: string; files: string[] }> {
   ...
   */
  private async writeFilesToFolder(
    folderPath: string,
   ...
   */
  async generateDirectoryTreeFromFilesystem(folderPath: string): Promise<string> {
    const folderName = path.basename(folderPath);
    const tree: string[] = [folderName + "/"];
   ...
   */
  async generateRunInstructions(folderPath: string): Promise<string[]> {
